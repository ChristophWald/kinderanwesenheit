...
TRAVEL_OPTIONS = ["Keine", "Übergabefahrt", "Arztbesuch"]
...
with left:
    ...
    if selected_user_id is None:
        ...
        default_start = time_options.index(existing["start_time"]) if existing else 16  # 08:00
        default_end = time_options.index(existing["end_time"]) if existing else 32      # 16:00
        default_comment = existing["comment"] if existing and existing["comment"] else ""
        existing_travel = existing["travel"] if existing and existing["travel"] else "Keine"
        default_travel = TRAVEL_OPTIONS.index(existing_travel) if existing_travel in TRAVEL_OPTIONS else 0

        start_time = st.selectbox("Beginn", time_options, index=default_start)
        end_time = st.selectbox("Ende", time_options, index=default_end)
        gemeinsam = st.checkbox("Gemeinsam")
        comment = st.text_input("Kommentar (optional)", value=default_comment)
        travel_selection = st.radio("Fahrt", TRAVEL_OPTIONS, index=default_travel, horizontal=True)
            ...
            if start_date > end_date:
                ...
                if df.empty:
                    ...
                    st.download_button(
                        ...
                        data=pdf_bytes,
                        file_name=f"auswertung_{selected_user}_{start_date}_{end_date}.pdf",
